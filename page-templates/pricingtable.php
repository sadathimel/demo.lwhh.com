<?php 

/*
 *   Template Name: Pricing Table
 */ 

get_header();
	$pricing = get_post_meta(get_the_ID(), "_alpha_pt_pricing_table",true);
	$services = get_post_meta(get_the_ID(), "_alpha_service",true);

?>
	

   <div class="container">
   	<div class="row">
   		<?php 
   			foreach ($pricing as $ptc) :
   		?>

   		<div class="col-md-4">
   			<h2><?php echo esc_html($ptc['_alpha_pt_priceing_caption']) ?></h2>
   			<ul>
   				<?php 
   					foreach ($ptc['_alpha_pt_pricing_option'] as $pto) :
   				?>
   				<li><?php echo esc_html($pto); ?></li>
   				<?php
   				endforeach;
   				?>
   			</ul>
   			<h3><?php echo esc_html($ptc['_alpha_pt_Price']); ?></h3>
   		</div>

   		<?php
   			endforeach;
   		?>
   	</div>

   	<div class="row">
   		<?php
   			foreach ($services as $srv) :
   				$alpha_service_icon = $srv['_alpha_icon'];
   		
   		?>

   		<div class="col-md-4">
   			<i class="fab <?php echo esc_attr( $alpha_service_icon ); ?>"></i>
   			<h2><?php echo esc_html($srv['_alpha_title']); ?></h2>
   			<?php echo apply_filters( "the_content", $srv['_alpha_content'] ); ?>
   		</div>	
   		
   		<?php
   		endforeach;	
   		 ?>
   	</div>
   </div>

<?php 
get_footer();


